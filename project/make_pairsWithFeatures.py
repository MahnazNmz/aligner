...
for line in onegram_file:
	...
	if f not in one_grams_e_f:
		one_grams_e_f[f] = {}
	one_grams_e_f[f][e] = prob
...
while e_line and f_line:
	...
	while len(f_line.strip()) != 0:
		for e_sent in english:
			...
			totalProb = 0
			for f_w in f_words:
				if f_w in one_grams_e_f:
					#find english translations of f_w that are present in the english sentences
					matches = (set(one_grams_e_f[f_w].keys()) & set(e_words))
					if len(matches) > 0:
						#add the probability from the best match
						totalProb += max([one_grams_e_f[f_w][e] for e in matches])
